#pragma once
// Lab08-jblakem5.cpp : This file contains the 'main' function. Program execution begins and ends there.
// BinaryTree.h
// Justin Blakeman
// COSC 2030
...
long BinaryTree::size(const BinaryNode * subtree){
	if (subtree == NULL){return 0;}			//Check if tree is empty
	else
	{
		return 1 + size(subtree->left_) + size(subtree->right_);		//Sums the size of the left subtree and the size of the right subtree (Start at 1 to Count Root)
	}
}
...
long BinaryTree::height(const BinaryNode * subtree) {
	if (subtree == NULL) { return 0; }			//Check if tree is empty
	else if (height(subtree->right_) > height(subtree->left_))	//If height of the right subtree is greater than the left
	{
		return 1 + height(subtree->right_);						//return 1 (root) + the height of the right subtree
	}
	else
	{
		return 1 + height(subtree->left_);						//return 1 (root) + the height of the left subtree
	}
}
...
long BinaryTree::leaves(const BinaryNode * subtree){
	...
	else if (subtree->left_ == NULL && subtree->right_ == NULL){			//If Both trees are empty then its just the root
		return 1;
	}

	else if (subtree->left_ != NULL && subtree->right_ != NULL){			//Redundant Check that seems to need to be there, idk.
		return leaves(subtree->left_) + leaves(subtree->right_);		//Sums leafs in each R/L-subtree
	}

	else if (subtree->right_ != NULL){									//if only right is not empty, can have in left without right.
...
short BinaryTree::leftmost(const BinaryNode * subtree){
	if (subtree == NULL) { return 0; }			//Check if tree is empty
		...
		return subtree->entry_;				//returns the leftmost node
	}
	else if (subtree->left_ != NULL){
		leftmost(subtree->left_);			//recurse of left node
	}
}
